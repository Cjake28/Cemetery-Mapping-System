...
export default function CreatePersonModal({ onClose, onCreatePerson }) {
  ...
  const validateForm = () => {
    ...
    if (!formData.center_lat_lng.trim()) {
      newErrors.center_lat_lng = "Center Latitude/Longitude is required";
    } else if (!/^-?\d+(\.\d+)?,\s*-?\d+(\.\d+)?$/.test(formData.center_lat_lng.trim())) {
      newErrors.center_lat_lng = "Invalid format for Latitude/Longitude";
    }
    return newErrors;
  };
      ...
      <div className="modal-content">
        ...
        <form onSubmit={handleSubmit} className="form-persons" autoComplete="off">
          ...
          <input
            ...
            id="name"
            name="name"
            className="modal-form-input-persons"
            value={formData.name}
          ...
          {errors.middle_name && <span className="error-text-persons">{errors.middle_name}</span>}
          <input
            type="text"
            id="middle_name"
            name="middle_name"
          ...
          <label htmlFor="center_lat_lng" className="modal-form-label-persons">Center Latitude/Longitude:</label>
          {errors.center_lat_lng && <span className="error-text-persons">{errors.center_lat_lng}</span>}
          <input
            type="text"
            id="center_lat_lng"
            name="center_lat_lng"
            className="modal-form-input-persons"
            value={formData.center_lat_lng}
            onChange={handleChange}
            placeholder="e.g., 14.888888852740292, 120.77931347934806"
            autoComplete="off"
          />

          <button type="submit" className="submit-btn-persons btn-persons">Create</button>
          <button type="button" className="cancel-btn-persons btn-persons" onClick={onClose}>Cancel</button>
        </form>
      </div>
    </div>
  );
